...
class AllEventsClassMD
{
     ...
     * @return string
     */
    public static function MakeMDObject($extension = "com_flexicontent", $object = "event")
    {
        ...
        $filename = JPATH_ROOT . '/documentation/docs/flexicontent/items/' . $object . '.md';
        $handle = fopen($filename, 'w');

        $sLine = '# flexicontent Object ' . $object;
     ...
     * @return string
     */
    public static function MakeMDConfig($extension = "com_flexicontent")
    {
        $lang = JFactory::getLanguage();
        $lang->load($extension, JPATH_ADMINISTRATOR, 'en-GB', true);
        $lang->load($extension . '.sys', JPATH_ADMINISTRATOR, 'en-GB', true);
        ...
        $filename = JPATH_ROOT . '/documentation/docs/flexicontent/config_' . $extension . '.md';
        $handle = fopen($filename, 'w');

        $sLine = '# flexicontent Component Configuration';
        fwrite($handle, $sLine . PHP_EOL . PHP_EOL);
                
        foreach ($get_xml->fields->fieldset as $fieldset) {
		// $sLine = print_r($get_xml,true);        
		// fwrite($handle, $sLine . PHP_EOL . PHP_EOL);
     ...
     */
    public static function MakeMDExtension($extension = "com_flexicontent", $name = "flexicontent")
    {
        $get_xml = simplexml_load_file(JPATH_ROOT . '/administrator/components/' . $extension . '/' . $name . '.xml');
        $filename = JPATH_ROOT . '/documentation/docs/flexicontent/' . $extension . '.md';
        $handle = fopen($filename, 'w');

        $sLine = '# flexicontent Component';
        fwrite($handle, $sLine . PHP_EOL . PHP_EOL);
        $sLine = '***';
        fwrite($handle, $sLine . PHP_EOL . PHP_EOL);
    ...
    public static function MakeMD($extension, $path, $subpath = "")
    {
        ...
        if ($path == 'modules') {
            ...
            $filename = JPATH_ROOT . '/documentation/docs/flexicontent/' . $path . '/' . JText::_($extension_name) . '.md';
        } elseif ($path == 'plugins') {
            $get_xml = simplexml_load_file(JPATH_ROOT . '/' . $path . '/' . $subpath . '/' . $extension . '/' . $extension . '.xml');
            $extension_name = $get_xml->name;
            if (empty($extension_name)) {
            ...
            $get_xml = simplexml_load_file(JPATH_ROOT . '/components/com_flexicontent/' . $path . '/' . $subpath . '/tmpl/' . $extension . '.xml');
            ...
            $filename = JPATH_ROOT . '/documentation/docs/flexicontent/' . $path . '/' . JText::_($extension_name) . '.md';
            $home = $get_xml;
            // $get_xml = $get_xml->metadata->fields;
        }
        ...
        if (!empty($filename)) {
            ...
            if ($path == 'views') {
				// $sLine = print_r($home, true);
                // fwrite($handle, $sLine . PHP_EOL);

                foreach ($home->fields as $fieldset) {
                    // if (!empty($fieldset->field)) {
                    $sLine = '### ' . JText::_($fieldset['name']);
                    fwrite($handle, $sLine . PHP_EOL);

                    
                foreach ($fieldset->fieldset as $fieldset) {
                    $sLine = ' > ' . JText::_($fieldset['name']);
                    fwrite($handle, $sLine . PHP_EOL. PHP_EOL);
$sLine = ' | Option | Description | Value |';
                    fwrite($handle, $sLine . PHP_EOL);
